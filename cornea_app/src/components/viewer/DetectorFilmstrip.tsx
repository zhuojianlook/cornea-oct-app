/* Detector FILMSTRIP (the former StepsViewer body, moved here verbatim — Steps v2 tab 2). The corneal-
   flattening algorithm's DEFAULT path shown as a top-down flow. The two surface detectors run IN PARALLEL —
   the native DP path and the legacy (old-method) RANSAC path — shown as two side-by-side branches that
   CONVERGE at the DP scar-guard cross-check (DP is kept in the vicinity of the legacy surface so it can't
   lock onto a bright internal scar). Below the merge: the shared per-slice stages and then the whole-volume
   decisions. Per-slice stages carry an image of ONE sagittal slice (pick which slice with the selector).
   NOTE: this replays the default pipeline from the raw .OCT (POST oct-preprocess-steps, heavy) — it is NOT
   what the last corrections run did; that lives in the "Run tree" tab. It is mounted lazily, only when the
   reviewer picks this tab, so the worker is never fired by merely opening Steps. */
...
export function DetectorFilmstrip() {
    ...
    <div className="flex flex-1 flex-col min-h-0 min-w-0" style={{ backgroundColor: "var(--c-bg)" }}>
      <div className="flex items-center gap-2 px-3 py-1 border-b flex-wrap"
        ...
        <span className="text-[11px]" style={{ color: "var(--c-text-dim)" }}>
          Default detector path replayed on the raw scan — both detectors run in parallel (red = detected border, green = legacy, blue = quadratic fit) and converge at the scar-guard.
        </span>
        {slices > 1 && (
