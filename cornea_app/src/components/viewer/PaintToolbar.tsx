/* Pen controls for the niivue drawing layer (cornea / background / scar / erase). */

import { ToggleButton, ToggleButtonGroup, Slider, Tooltip } from "@mui/material";
import { usePaintStore, type PenLabel } from "../../store/paintStore";
import { setDrawOpacity } from "../../niivue/nvController";

const PENS: { value: PenLabel | 3; label: string; color: string }[] = [
...
export function PaintToolbar() {
  const { penLabel, drawingLoaded, drawOpacity, setPenLabel, set } = usePaintStore();
  if (!drawingLoaded) return null;
  ...
  return (
    <div className="flex items-center gap-3 px-3 border-b" style={{ height: 36, borderColor: "var(--c-border)" }}>
      ...
      <ToggleButtonGroup
        ...
        exclusive
        value={penLabel}
        onChange={(_, v) => v !== null && setPenLabel(v)}
      >
        {PENS.map((p) => (
      ...
      <Tooltip title="Seed paint opacity">
