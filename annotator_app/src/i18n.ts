...
const T = {
  ...
  "tb.smartTip":  { en: "Smart fill = Slicer “Grow from seeds” (GrowCut): scribble a little Cornea, Scar AND Background on a few slices, then every unlabelled voxel is assigned to the nearest seed by intensity similarity. Background seeds stop the growth at a real boundary, so the region doesn't flood the whole volume.",
                    zh: "智能填充 = Slicer“从种子生长”（GrowCut）：在若干切片上分别涂少量角膜、瘢痕与背景种子，每个未标注体素将按强度相似度归入最近的种子。背景种子让生长在真实边界处停止，避免填满整个体数据。" },
